...
4. **新增市場智慧層**：掃描器抓到活躍市場後「先寫進 database」，跑完後用新聞/研究給分也寫回 DB。下次執行時低分市場會被**廉價跳過**（大幅減少重複燒 token），同時讓 agent 主動探索更多類型市場。
5. 任何時候本金低於初始值或 API 預算快用完 → 立即停止交易
...
- **新增「微型探索倉位」機制**（僅限研究很弱時）：在嚴格保護本金的前提下，允許提出 $0.10–0.30 的極小倉位作為資料蒐集用途。主體決策依然極度保守，只有這個通道會放寬門檻。
